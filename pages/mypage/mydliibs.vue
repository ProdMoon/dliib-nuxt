...
const dliibs = ref<Dliib[]>(
  await useApiFetch('/api/dliib/my', {
    method: 'GET',
    headers: {
...
<template>
  <ButtonNavigateMenu v-for="dliib in dliibs" :href="`/mypage/mydliib/${dliib.id}`">
    <div class="truncate">{{ dliib.contents?.[0] }}</div>
    <div class="text-gray-500 text-xs mt-1 flex items-center space-x-2">
      <div>{{ momentFormat(dliib.createdAt) }}</div>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-4 text-blue-300">
        <path
